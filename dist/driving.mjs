...
export function defaultProfile(){return{credits:0,best:0,runs:0,deliveries:0,unlocked:['van'],selected:'van',paint:0,muted:false,camera:'chase',tutorialStep:0,homeVersion:HOME_VERSION,home:{...HOME_SPAWN},homeLife:{tvOn:false,completed:[]},checkpoint:null,collection:defaultCollection()};}
export function cleanProfile(raw){const p=defaultProfile();if(!raw||typeof raw!=='object')return p;for(const k of ['credits','best','runs','deliveries'])if(Number.isFinite(raw[k]))p[k]=clamp(Math.floor(raw[k]),0,1e9);p.collection=cleanCollection(raw.collection);p.unlocked=['van',...Object.values(BURN_CARS).filter(c=>p.collection.owned.includes(c.itemId)).map(c=>c.id),...['coupe','racer'].filter(k=>Array.isArray(raw.unlocked)&&raw.unlocked.includes(k))];if(p.unlocked.includes(raw.selected))p.selected=raw.selected;p.paint=Number.isInteger(raw.paint)?clamp(raw.paint,0,PAINTS.length-1):0;p.muted=raw.muted===true;p.camera=raw.camera==='high'?'high':'chase';p.tutorialStep=Number.isInteger(raw.tutorialStep)?clamp(raw.tutorialStep,0,6):0;if(raw.homeVersion===HOME_VERSION&&Number.isFinite(raw.home?.x)&&Number.isFinite(raw.home?.z))p.home={x:clamp(raw.home.x,HOME_BOUNDS.minX,HOME_BOUNDS.maxX),z:clamp(raw.home.z,HOME_BOUNDS.minZ,HOME_BOUNDS.maxZ)};p.homeLife={tvOn:raw.homeLife?.tvOn===true,completed:Array.isArray(raw.homeLife?.completed)?['sofa','coffee','shower','bed'].filter(id=>raw.homeLife.completed.includes(id)):[]};p.checkpoint=raw.checkpoint?.version===1?raw.checkpoint:null;return p;}
