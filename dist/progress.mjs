import {passengerJob,cleanRide} from './passengers.mjs?v=14';
import {CARS,STOPS,DESTS,LIMIT,clamp,createCar,createRun} from './driving.mjs?v=14';
...
export function captureShift(player,run,pickups,locks={}){
 ...
 return{version:2,rides:run.passengers.map(p=>({id:p.id,...p.ride})),carId:player.config.id,car,run:{...Object.fromEntries(RUN_FIELDS.map(k=>[k,run[k]])),passengerIds:run.passengers.map(j=>j.id)},jobs:pickups.map(p=>job(p.job)),locks:{bankRoute:!!locks.bankRoute,pickupLock:locks.pickupLock||null,targetLock:locks.targetLock||null}};
}
