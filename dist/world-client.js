...
export function createNeighborhood(api){
 ...
 async function join(id){const result=await request('/join','POST',id?{roomId:id}:{});session=result.session;roomId=result.roomId;data.profile=result.profile;data.home=result.home;seq=0;lastPoll=Date.now();failures=0;error='';clearPeers();applyHome();await load();api.notify('WELCOME HOME · '+address());}
 ...
 function disposePeer(p){api.world.remove(p.view.group);p.view.root.traverse(o=>{if(o.isMesh)for(const m of Array.isArray(o.material)?o.material:[o.material])m.dispose();});p.label.material.map.dispose();p.label.material.dispose();}
 ...
 function updatePeers(list){
  ...
  for(const p of peers){let item=views.get(p.id);if(item&&(item.carId!==p.carId||item.name!==p.name)){disposePeer(item);views.delete(p.id);item=null;}
   if(!item){const cfg=CARS[p.carId]||CARS.van,view=makeVehicle(DETAILED_STAND_INS[cfg.model]||cfg.model,cfg.scale,PAINTS[p.id.charCodeAt(0)%PAINTS.length]);view.group.userData.playerId=p.id;api.world.add(view.group);const tag=label(p.name);view.group.add(tag);item={view,label:tag,carId:p.carId,name:p.name,current:{x:p.x,z:p.z,heading:p.heading},target:p};views.set(p.id,item);}item.target=p;
  }
 ...
 async function sync(){
  ...
  try{await interiorUpdate;const s=api.state();const result=await request('/sync','POST',{session:token,seq:++seq,x:s.x,z:s.z,heading:s.heading,mode:s.mode,carId:s.carId});if(token!==session)return;data.home=result.home;applyHome();updatePeers(result.players);failures=0;error='';}
 ...
 function update(dt){const now=Date.now();if(session&&!syncing&&now-lastPoll>1200&&now>=retryAt&&!document.hidden){lastPoll=now;void sync();}const s=api.state();for(const item of views.values()){const t=item.target,c=item.current,f=1-Math.exp(-dt*7);c.x+=(t.x-c.x)*f;c.z+=(t.z-c.z)*f;c.heading+=Math.atan2(Math.sin(t.heading-c.heading),Math.cos(t.heading-c.heading))*f;const moving=dist(c,t)>.1;item.view.group.visible=t.mode==='driving'&&s.mode==='driving'&&dist(s,c)<160;animateVehicle(item.view,{...c,y:0,vy:0,yawRate:0,speed:moving?8:0,steering:0,drifting:false,braking:!moving,boosting:false,config:CARS[item.carId],wheelTravel:now*.008},dt);}refreshHUD();}
 $('worldButton').onclick=()=>open();
 addEventListener('pagehide',()=>{disposed=true;if(session)void request('/leave','POST',{session},true).catch(()=>{});});
 function enterInterior(mode){if(!session)return;const token=session;interiorUpdate=interiorUpdate.catch(()=>{}).then(()=>request('/interior','POST',{session:token,mode})).catch(()=>{});lastPoll=Date.now()+400;}
 return {open,update,load:()=>load().catch(()=>{}),enterInterior,connected:()=>!!session,peers:()=>peers,room:()=>roomId,home:()=>data.home,place,job:()=>data.home?.job,trackJob,state:()=>({connected:!!session,roomId,players:peers.length+(session?1:0),home:data.home,error}),join,leave};
}
