import {addHomeSurroundings,createHomeDesign} from './home-design.js?v=38';
import {createWalkingView} from './first-person.js?v=38';
import * as THREE from './assets/three.module.js';
import {HOME_SPAWN,HOME_SPOTS,HOME_BOUNDS,homeRoom} from './home-layout.mjs?v=38';
export {HOME_SPAWN,HOME_SPOTS} from './home-layout.mjs?v=38';
...
export function buildApartment(templates,position=HOME_SPAWN,life={}){
 ...
 const avatar=new THREE.Group();avatar.position.set(position.x,.36,position.z);scene.add(avatar);let character,mixer,actions={},currentAnimation='',emoteUntil=0,emoteName='';
 ...
 const radius=.23;
 function blocked(x,z){return !Number.isFinite(x)||!Number.isFinite(z)||x<HOME_BOUNDS.minX||x>HOME_BOUNDS.maxX||z<HOME_BOUNDS.minZ||z>(homeLevel>=3?15.68:HOME_BOUNDS.maxZ)||colliders.some(b=>x+radius>b.x-b.w/2&&x-radius<b.x+b.w/2&&z+radius>b.z-b.d/2&&z-radius<b.z+b.d/2);}
 ...
 function setFirstPerson(value){if(typeof studioCeiling!=='undefined')studioCeiling.visible=!!value;fpShell.visible=!!value;viewMode=value?'first':'room';walkingView.set(value);resize(viewport.width,viewport.height);}
 ...
 function positionRoomCamera(smooth){const focus=avatar.position,wide=viewport.width/viewport.height>1.9?.88:1;const narrow=viewport.width/viewport.height<.85;roomLook.set(THREE.MathUtils.clamp(focus.x,narrow?-10:-7.7,narrow?10:7.75),.85,THREE.MathUtils.clamp(focus.z,-5.7,homeLevel>=3?14:narrow?7.1:4.4));desiredEye.set(roomLook.x+5.1*zoom*wide,roomLook.y+6.65*zoom*wide,roomLook.z+7.0*zoom*wide);camera.position.lerp(desiredEye,smooth);camera.lookAt(roomLook);camera.updateMatrixWorld();}
 function resize(width,height){viewport={width,height};camera.aspect=width/height;camera.clearViewOffset();camera.fov=44;camera.updateProjectionMatrix();if(walkingView.enabled){walkingView.set(true);return;}if(viewMode==='room'){if(width<=650)camera.setViewOffset(width,height,0,-height*.16,width,height);positionRoomCamera(1);return;}
  let distance=.6;for(let i=0;i<9;i++){camera.position.set(15.5,19.5,22).multiplyScalar(distance);camera.lookAt(0,1,0);camera.updateMatrixWorld();const points=[...corners,...(homeLevel>=3?[new THREE.Vector3(-10.8,0,16),new THREE.Vector3(10.8,0,16)]:[])].map(v=>v.clone().project(camera)),ratio=Math.max((Math.max(...points.map(v=>v.x))-Math.min(...points.map(v=>v.x)))/1.9,(Math.max(...points.map(v=>v.y))-Math.min(...points.map(v=>v.y)))/1.76);if(ratio<=1)break;distance*=ratio*1.01;}
 ...
 function update(dt,input={},step=6,time=0){const before=avatar.position.clone(),manual=!!(input.right||input.left||input.down||input.up);if(manual){if(activity)stopActivity(activity.id==='sofa');routePoints=[];routeTarget=null;arrived=null;}
  ...
  const moved=before.distanceTo(avatar.position);if(!activity)playAnimation(Date.now()<emoteUntil?(emoteName==='dance'?'walk':'emote-yes'):moved>.0001?'walk':'idle');mixer.update(dt);
 ...
 const studio=new THREE.Group();studio.name='Penthouse studio expansion';room.add(studio);studio.visible=false;
 box(21.8,.42,8.2,'#31483d',0,.04,12,{},studio);box(21.6,.1,8,'#b3986a',0,.3,12,{},studio);
 for(let row=0;row<20;row++)box(21.4,.02,.025,'#77674e',0,.36,8.1+row*.4,{},studio);
 for(const x of [-10.8,10.8]){box(.18,1.1,8,'#728873',x,.9,12,{},studio);box(.24,.1,8.15,'#d1c8a5',x,1.5,12,{},studio);}
 box(21.6,.8,.18,'#667f73',0,.72,16,{},studio);box(21.7,.12,.26,'#d1c8a5',0,1.18,16,{},studio);
 const studioCeiling=box(21.6,.12,8,'#d5d6c3',0,3.68,12,{},studio);studioCeiling.visible=false;
 for(const [name,x,z,w]of [['pottedPlant',-9.4,9.4,1],['loungeChairRelax',-7,12,1.6],['tableCoffeeGlass',-5,12,1.3],['radio',-5,12,.5],['plantSmall1',8.8,14.5,.8]]){const p=prop(name,x,z,w,{collision:false,y:name==='radio'?1.1:.35});p.group.userData.sunroom=true;studio.add(p.group);}
 const studioLight=new THREE.PointLight('#ffe7b3',28,20,2);studioLight.position.set(0,3.1,12);studio.add(studioLight);
 const studioSign=label('THE SUNROOM',4,.5,0,2.2,15.85,Math.PI,'#fff1b3','#365545');studio.add(studioSign);
 function setHomeLevel(level,unit){const previousLevel=homeLevel;homeLevel=Math.max(0,Math.min(3,Number(level)||0));homeLayers.forEach((g,i)=>g.visible=homeLevel>i);studio.visible=homeLevel>=3;for(let i=colliders.length-1;i>=0;i--)if(colliders[i].sunroom)colliders.splice(i,1);if(homeLevel>=3)for(const p of furniture.filter(p=>p.group.userData.sunroom&&p.name!=='radio')){const b=p.bounds;colliders.push({x:(b.min.x+b.max.x)/2,z:(b.min.z+b.max.z)/2,w:b.max.x-b.min.x,d:b.max.z-b.min.z,sunroom:true,name:p.name});}for(const m of occluders)if(m.userData.homeAction==='door')m.visible=homeLevel<3;if(homeLevel<3&&avatar.position.z>7.68)setPosition(HOME_SPAWN);floor.material.color.set(homeLevel>=3?'#d6c4a7':'#a28161');if(previousLevel!==homeLevel)resize(viewport.width,viewport.height);if(unit){const c=addressLabel.material.map.image,ctx=c.getContext('2d');ctx.fillStyle='#2d4852';ctx.fillRect(0,0,c.width,c.height);ctx.fillStyle='#e6c784';ctx.textAlign='center';ctx.textBaseline='middle';ctx.font='700 80px monospace';ctx.fillText(String(unit).padStart(3,'0'),c.width/2,c.height/2,c.width-30);addressLabel.material.map.needsUpdate=true;}}
 const design=createHomeDesign({scene,room,floor,walls:[...occluders.filter(m=>!m.userData.homeAction),...fpShell.children]});
 return {emote(id){emoteName=id;emoteUntil=Date.now()+3200;},setDecor:(decor,reward)=>{design.apply(decor,reward);baseRug.forEach(m=>m.visible=false);homeLayers[0].children.slice(0,11).forEach(m=>m.visible=false);},scene,camera,avatar,walkingView,studio,setHomeLevel,homeLayers,get homeLevel(){return homeLevel;},setFirstPerson,look:walkingView.look,colliders,hotspots,furniture,update,nearest,canInteract,setPosition,blocked,resize,setCharacter,toggleView,zoomBy,interact,walkTo,clickAt,stopActivity,
