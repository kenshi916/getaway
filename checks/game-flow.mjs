...
const oldSave={...core.defaultProfile(),homeVersion:undefined,home:{x:5,z:-4},credits:7100,unlocked:['van','coupe'],selected:'coupe',tutorialStep:5};const migrated=core.cleanProfile(oldSave);assert.deepEqual(migrated.home,HOME_SPAWN);assert.equal(migrated.credits,7100);assert.equal(migrated.selected,'coupe');assert.equal(migrated.tutorialStep,5);
...
for(const [w,h]of [[1280,800],[390,844],[375,667],[320,568],[844,390]]){home.resize(w,h);home.camera.updateMatrixWorld();for(const x of [-7.6,7.6])for(const z of [-5.6,5.6]){const p=new RealThree.Vector3(x,0,z).project(home.camera);assert(Math.abs(p.x)<1&&Math.abs(p.y)<1,'room floor fits '+w+'x'+h);}}home.resize(1280,800);
assert(home.furniture.length>=40,'modeled interior furniture loads');for(const prop of home.furniture)assert(prop.bounds.min.x>=-7.6&&prop.bounds.max.x<=7.6&&prop.bounds.min.z>=-5.6&&prop.bounds.max.z<=5.6,'furniture stays inside room: '+prop.name);for(const prop of home.furniture){assert(!prop.bounds.isEmpty(),'furniture has geometry');assert(prop.bounds.min.toArray().concat(prop.bounds.max.toArray()).every(Number.isFinite),'finite furniture bounds');}
ids.get('homeView').onclick();assert.equal(home.viewMode,'room');for(let i=0;i<5;i++)callback();assert(home.camera.position.toArray().every(Number.isFinite));ids.get('homeView').onclick();assert.equal(home.viewMode,'overview');
...
assert(reachable.some(p=>Math.hypot(p.x-5.8,p.z-3.2)<.4),'bathroom floor is reachable');
for(const point of [{x:3.05,z:.85},{x:4.5,z:3.2}])assert(!home.blocked(point.x,point.z),'doorway is open');
for(const id of ['tv','coffee']){const spot=HOME_SPOTS.find(s=>s.id===id),p=reachable.find(p=>Math.hypot(p.x-spot.x,p.z-spot.z)<.4);assert(p,'approach '+id);home.setPosition(p);b.interact();assert(ids.get('toast').textContent.includes(id==='tv'?'TV ON':'COFFEE'));}
home.setPosition(HOME_SPAWN);
assert(home.blocked(4.42,-3.53),'bed blocks movement');assert(home.blocked(8,2),'room boundary blocks movement');
...
home.setPosition(HOME_SPOTS.find(s=>s.id==='bed'));b.interact();assert(ids.get('modalRoot').innerHTML.includes('ALL SAVED.'));b.close();
