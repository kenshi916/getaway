...
const saved=new Map();let callback;const context={...core,THREE:{...RealThree,WebGLRenderer:Renderer,Clock:class{getDelta(){return 1/60}}},GLTFLoader:Loader,mergeGeometries,makeVehicle,animateVehicle,loadVehiclePack,loadCityPack,buildCity,buildApartment,HOME_SPAWN,captureShift,restoreShift,document,localStorage:{getItem:k=>saved.get(k),setItem:(k,v)=>saved.set(k,v)},matchMedia:()=>({matches:false}),devicePixelRatio:1,innerWidth:1280,innerHeight:800,addEventListener(){},requestAnimationFrame:f=>{callback=f},setTimeout(){},console,AbortController,Uint8Array,Uint8ClampedArray};context.window=context;vm.createContext(context);
const source=fs.readFileSync(new URL('../dist/getaway.js',import.meta.url),'utf8').replace(/^import .*\n/gm,'');
const application=source+`;this.bridge={start:startRun,garage:showGarage,close:closeModal,pause:showPause,jobs:showJobs,bank,menu:returnMenu,route:toggleGarageRoute,get:()=>({ready,mode,paused,player,run,profile,bankReady,pickups,cityState,residents,apartment,bankRoute}),interact:interactHome,leave:leaveApartment,resume:resumeRun,save:saveProfile,input:(value)=>{keys=value},reloadProfile:()=>{profile=cleanProfile(JSON.parse(localStorage.getItem('getaway-profile-v1')))},teleport:(p)=>{Object.assign(player,{x:p.x,z:p.z,vx:0,vz:0,speed:0,y:0,vy:0,yawRate:0,steering:0});},fund:()=>{profile.credits=20000}}`;vm.runInContext(application,context);
...
b.fund();b.garage();let card=document.querySelectorAll('[data-car]').find(x=>x.dataset.car==='coupe');assert(card.onclick);card.onclick();assert.equal(b.get().profile.selected,'coupe');assert.equal(b.get().profile.credits,14000);const paint=document.querySelectorAll('[data-paint]').find(x=>x.dataset.paint==='2');paint.onclick();assert.equal(b.get().profile.paint,2);ids.get('garageDrive').onclick();assert.equal(b.get().mode,'apartment');b.leave();assert.equal(b.get().player.config.id,'coupe');b.pause();assert(b.get().paused);b.close();for(let i=0;i<5;i++)callback();
...
assert(fresh.get().ready);assert.equal(fresh.get().mode,'apartment');assert.equal(fresh.get().profile.tutorialStep,6);assert.equal(fresh.get().profile.credits,14000);assert.equal(fresh.get().profile.checkpoint.carId,'coupe');
ids.get('homeResume').onclick();assert.equal(fresh.get().mode,'driving');assert.equal(fresh.get().player.config.id,'coupe');assert.equal(fresh.get().player.health,67);assert.equal(fresh.get().player.nitro,42);assert.equal(fresh.get().run.wanted,2);assert.equal(fresh.get().run.time,beforeReload.checkpoint.run.time);
...
const cards=document.querySelectorAll('[data-car-preview]');assert.equal(cards.length,3);for(const card of cards)assert(card.children[0].src.startsWith('data:image/png'),'actual car preview created');
...
console.log('Website simulation passed: city loads, three model previews render, district buttons change views, and scene transforms stay finite.');
