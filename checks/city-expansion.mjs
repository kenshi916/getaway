...
assert.equal(ROAD.length,16);assert.equal(BLOCKS.filter(b=>b.outer).length,200);assert.equal(LANDMARKS.length,8);
...
for(const bad of [{x:Infinity,z:0},{x:300,z:0},{x:0,z:0}]){assert.equal(restoreShift({...raw,locks:{mapWaypoint:bad}},p).locks.mapWaypoint,null)}
...
console.log('Expanded city verified: 168 outer routes clear, trapped legacy saves recovered, 5-minute shifts and waypoints restored, invalid waypoint rejection, authored stories preserved, Pons launch gate closed.');
