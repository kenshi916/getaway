...
## Garage environment and blocky Web3 website

The garage uses a new `garage-room.js` environment: concrete panels, inset shutter, correct-scale workbench and tool drawers, pegboard tools, tire storage, marked parking bays, warm overhead lights, and indoor reflections. Furniture materials are cloned before recoloring, preserving apartment assets. Direct scenery meshes are batched while prop groups retain their collision bounds. Car browsing, demo burns, equipment, walking, and saved shifts retain their existing behavior. The garage UI uses clearer controls, car details, and a compact six-car strip.
...
## Walkable destination quests

All 22 passenger destinations now launch a story-specific visit after a grounded, stationary arrival. Four reusable 3D environments provide apartments, garages, conservatories, and music/café venues, with real imported furniture, separate passenger/driver characters, collision-aware walking, sliding front doors or rising garage shutters, and a marked handoff counter. Open the entrance, perform the handoff, and speak with the passenger before `deliver()` settles the fare. Phone → Quests lists onboard passengers’ destination quests and routes to them.

`destination-quests.mjs` maps destinations and passenger tasks; `destination-scene.js` builds the scene and walking interactions; `destination.css` handles the visit UI and success screen. Existing models are reused without downloads. Floor tiles are instanced, and exit disposes owned geometry, materials, textures, shadow targets and instance buffers while preserving shared templates.

Destination mode freezes the shift, pursuit and all onboard ride goals. Checkpoint capture includes this active-shift mode, so passenger removal, haul, time and story history save before the success prompt. An unfinished visit saves the passenger and restarts at the entrance after reload. Per-passenger dismissals prevent automatic re-entry loops, including co-located riders; eligible passengers can complete a visit while a stealth rider waits. Completion guards and stale-callback checks prevent duplicate settlement. Fare, tip, goal bonuses and up-to-40-second additions retain their existing rules; no new currency or live-token behavior was added.

The full game simulation passes once-only rewards, saved progress before/after completion, frozen timers, abort/re-entry, shared destinations, grounded entry, stale result callbacks and existing garage/phone/website flows. `node checks/destinations.mjs` checks real-model navigation, closed/open door collision, all interaction paths, NPC clearance and resource disposal across all four themes. Desktop and emulated touch-mobile Edge QA verifies visible door animation, walking and release, handoffs, conversations, success prompts and return to the car, without JavaScript or shader errors. Typical tested interior scenes draw 93–129 calls and about 7,000–8,800 triangles; physical-device frame rates remain unmeasured.


## Detailed concept car lineup

All six playable rides now use detailed concept cars. The user's four Unity Fan downloads supply Concept 011 (Night Runner), 050 (Redline), 005 (Workhorse utility and Night Cab taxi trim), and 006 (Ironhide). Alley Cat retains its existing Khronos CarConcept model. Stable car IDs, unlock prices, burn item IDs, handling stats and saved progress are preserved. Original downloaded license files are included beside the models; the files permit commercial derivative works under Sketchfab Standard and their titles identify the author's CC0 release. Alley Cat retains its CC BY 4.0 credit.
