...
export async function worldApi(request,{db,me,body,json,fail,url}){
 ...
 if(path==='/api/world/sync'&&method==='POST'){
  ...
  const venue=b.venue===CLUB.id&&b.mode==='destination'?CLUB.id:'';if(venue&&presence?.venue!==CLUB.id)fail('Enter the club through its front door.',409);
